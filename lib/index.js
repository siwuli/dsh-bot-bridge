...
export function apply(ctx, config) {
  ...
  const cfg = {
    ...
    defaultModelProvider: typeof config?.defaultModelProvider === 'string' && config.defaultModelProvider.trim()
      ? config.defaultModelProvider.trim()
      : undefined,
    defaultModelName: typeof config?.defaultModelName === 'string' && config.defaultModelName.trim()
      ? config.defaultModelName.trim()
      : undefined,
    defaultReasoningEffort: typeof config?.defaultReasoningEffort === 'string' && config.defaultReasoningEffort.trim()
      ? config.defaultReasoningEffort.trim()
      : undefined,
  };

  const store = core.loadStore(cfg.storageFile);
  // 多实例(如另开的测试实例)共享同一存储文件: 保存时与磁盘合并,
  // 内存里的值优先, 显式删除的 key 用墓碑移除 —— 避免旧进程的过期映射
  // 覆盖新进程刚写入的绑定, 导致 clientId 绑定丢失、每条消息都新建会话。
  const deletedKeys = new Set();
  const persist = () => {
    const merged = { ...core.loadStore(cfg.storageFile), ...store };
    for (const key of deletedKeys) delete merged[key];
    core.saveStore(cfg.storageFile, merged);
  };
  ...
  /** 确保工作区存在: 建目录 → 注册/复用 DSH 工作区 → 返回 canonical path + workspaceId */
  const ensureWorkspace = async (path) => {
    try { mkdirSync(path, { recursive: true }); } catch (_) { /* 目录已存在/不可写时交给下方报错 */ }
    try {
      const result = unwrap(await ctx.apiProxy.workspace.create({ rpcId: newRpcId(), payload: { path } }));
      if (result?.ok === true && result.value?.workspace?.workspaceId) {
        return { workspaceId: result.value.workspace.workspaceId, path: result.value.workspace.path ?? path };
      }
      ctx.logger?.warn?.('dsh-bot-bridge: workspace create failed: ' + JSON.stringify(result?.error ?? {}));
    } catch (error) {
      ctx.logger?.warn?.('dsh-bot-bridge: workspace ensure failed: ' + String(error));
    }
    return { workspaceId: undefined, path };
  };
  ...
  const handleSessions = async (req, res) => {
    ...
    const result = unwrap(await sessions().list({ rpcId: newRpcId(), payload: {} }));
    if (result?.ok !== true) {
      const e = result?.error ?? {};
      return reject(res, 502, e.code ?? 'internal', 'session list failed: ' + (e.message ?? 'unknown'));
    }
    const items = (result.value?.items ?? [])
      .filter((s) => s?.origin !== 'subagent')
      .filter((s) => !workspacePath || (typeof s?.cwd === 'string'
        && core.normalizePath(s.cwd) === core.normalizePath(workspacePath)))
      .map((s) => ({
    ...
    core.writeJson(res, { ok: true, items });
  };

  const handleBind = async (req, res) => {
    if (!guarded(req, res)) return;
    let body;
    try { body = await core.readJsonBody(req); } catch (e) {
      return reject(res, 400, e.code ?? 'bad-request', e.message);
    ...
    const clientId = typeof body?.clientId === 'string' ? body.clientId.trim() : '';
    const sessionId = typeof body?.sessionId === 'string' ? body.sessionId.trim() : '';
    if (!clientId || !sessionId) return reject(res, 400, 'bad-request', 'clientId and sessionId are required');
    // 校验会话存在
    const listed = unwrap(await sessions().list({ rpcId: newRpcId(), payload: {} }));
    const hit = (listed?.value?.items ?? []).find((s) => s?.sessionId === sessionId);
    if (!hit) return reject(res, 400, 'session-not-found', 'no such session: ' + sessionId);
    store[clientId] = sessionId;
    deletedKeys.delete(clientId);
    persist();
    core.writeJson(res, {
      ok: true,
      sessionId,
      cwd: hit.cwd ?? null,
      title: hit.projections?.values?.title ?? null,
    });
  };

  const handleWorkspaces = async (req, res) => {
    if (!guarded(req, res)) return;
    const result = unwrap(await ctx.apiProxy.workspace.list({ rpcId: newRpcId(), payload: {} }));
    if (result?.ok !== true) {
      const e = result?.error ?? {};
      return reject(res, 502, e.code ?? 'internal', 'workspace list failed: ' + (e.message ?? 'unknown'));
    }
    const items = (result.value?.items ?? []).map((ws) => ({
  ...
  const handleModels = async (req, res) => {
    if (!guarded(req, res)) return;
    const result = unwrap(await ctx.apiProxy.llm.models({ rpcId: newRpcId(), payload: {} }));
    if (result?.ok !== true) {
      const e = result?.error ?? {};
      return reject(res, 502, e.code ?? 'internal', 'model catalog failed: ' + (e.message ?? 'unknown'));
    }
    const groups = (result.value?.groups ?? []).map((g) => ({
      provider: g.id,
      name: g.name,
    ...
    core.writeJson(res, {
      ...
      defaultModelProvider: cfg.defaultModelProvider ?? null,
      defaultModelName: cfg.defaultModelName ?? null,
    });
  };

  const handleModel = async (req, res) => {
    if (!guarded(req, res)) return;
    let body;
    try { body = await core.readJsonBody(req); } catch (e) {
      return reject(res, 400, e.code ?? 'bad-request', e.message);
    ...
    if (!sessionId || !provider || !model) {
      return reject(res, 400, 'bad-request', 'sessionId, provider and model are required');
    }
    const result = unwrap(await sessions().selectModel({
      rpcId: newRpcId(),
      payload: {
        sessionId,
        provider,
    ...
    if (result?.ok !== true) {
      const e = result?.error ?? {};
      return reject(res, 502, e.code ?? 'internal', 'select model failed: ' + (e.message ?? 'unknown'));
    }
    core.writeJson(res, { ok: true, selected: result.value?.selected ?? null });
  };
  ...
  const handleCancel = async (req, res) => {
    ...
    const sessionId = typeof body?.sessionId === 'string' ? body.sessionId.trim() : '';
    if (!sessionId) return reject(res, 400, 'missing-session-id', 'sessionId is required');
    const result = unwrap(await sessions().cancel({ rpcId: newRpcId(), payload: { sessionId } }));
    if (result?.ok !== true) {
      const e = result?.error ?? {};
      return reject(res, 502, e.code ?? 'internal', 'cancel failed: ' + (e.message ?? 'unknown'));
    }
    core.writeJson(res, { ok: true, accepted: true });
  };
  ...
  const handlePrompt = async (req, res) => {
    ...
    if (workspacePath && !core.isAbsolutePath(workspacePath)) {
      return reject(res, 400, 'invalid-workspace-path',
        'workspacePath must be an absolute path (e.g. D:/work/x or /data/x), got "' + workspacePath + '"');
    }
    ...
    // 工作区自动换绑: 绑定会话的工作目录与请求/配置的工作区不一致时,
    // 解绑旧会话并新建 (否则 workspacePath 配置"改了不生效")。
    let wsInfo;
    if (workspacePath) wsInfo = await ensureWorkspace(workspacePath);
    if (sessionId && !sessionIdProvided && wsInfo) {
      try {
        const sessionsSvc = ctx.get('sessions');
        const attachedCwd = sessionsSvc?.get?.(sessionId)?.header?.cwd;
        let currentCwd = attachedCwd;
        if (typeof currentCwd !== 'string') {
          const persistence = ctx.get('sessionPersistence');
          const inspected = typeof persistence?.inspect === 'function'
            ? await persistence.inspect(sessionId)
            : undefined;
          currentCwd = inspected?.meta?.cwd;
        }
        if (typeof currentCwd === 'string' && core.normalizePath(currentCwd) !== core.normalizePath(wsInfo?.path ?? workspacePath)) {
          delete store[clientId];
          deletedKeys.add(clientId);
          persist();
          sessionId = undefined;
        }
      } catch (_) { /* 无法确认 cwd 时保持复用 */ }
    }
    if (!sessionId) {
      const created = unwrap(await sessions().create({
        rpcId: newRpcId(),
        payload: {
          ...(wsInfo?.workspaceId ? { workspaceId: wsInfo.workspaceId } : wsInfo ? { cwd: wsInfo.path } : {}),
      ...
      // 新建会话时应用模型选择 (请求字段优先, 其次插件配置默认值)
      const modelProvider = (typeof body?.modelProvider === 'string' && body.modelProvider.trim()
        ? body.modelProvider.trim()
        : undefined) ?? cfg.defaultModelProvider;
      const modelName = (typeof body?.modelName === 'string' && body.modelName.trim()
        ? body.modelName.trim()
        : undefined) ?? cfg.defaultModelName;
      if (modelProvider && modelName) {
        const effort = (typeof body?.reasoningEffort === 'string' && body.reasoningEffort.trim()
          ? body.reasoningEffort.trim()
          : undefined) ?? cfg.defaultReasoningEffort;
        const payload = { sessionId, provider: modelProvider, model: modelName };
        if (effort) payload.reasoningEffort = effort;
        const selected = unwrap(await sessions().selectModel({ rpcId: newRpcId(), payload }));
        if (selected?.ok !== true) {
          ctx.logger?.warn?.('dsh-bot-bridge: apply model selection failed: ' + JSON.stringify(selected?.error ?? {}));
        }
      }
    }
  ...
  const router = async (req, res) => {
    ...
    switch (pathname) {
      ...
      case '/api/bot/sessions': return req.method === 'GET' ? handleSessions(req, res) : reject(res, 405, 'method-not-allowed', 'GET only');
      case '/api/bot/bind': return req.method === 'POST' ? handleBind(req, res) : reject(res, 405, 'method-not-allowed', 'POST only');
      case '/api/bot/workspaces': return req.method === 'GET' ? handleWorkspaces(req, res) : reject(res, 405, 'method-not-allowed', 'GET only');
      case '/api/bot/models': return req.method === 'GET' ? handleModels(req, res) : reject(res, 405, 'method-not-allowed', 'GET only');
      case '/api/bot/model': return req.method === 'POST' ? handleModel(req, res) : reject(res, 405, 'method-not-allowed', 'POST only');
      ...
      case '/api/bot/cancel': return req.method === 'POST' ? handleCancel(req, res) : reject(res, 405, 'method-not-allowed', 'POST only');
