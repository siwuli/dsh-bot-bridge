...
export function apply(ctx, config) {
  ...
  const handlePrompt = async (req, res) => {
    ...
    const maybeScheduleDone = () => {
      if (closed) return;
      if (doneTimer) clearTimeout(doneTimer);
      if (cfg.doneQuietMs <= 0) { close(); return; }
      doneTimer = setTimeout(() => close(), cfg.doneQuietMs);
    };
          ...
          if (payload.type === 'session/event') {
            ...
            if (event.type === 'assistant/chunk') {
              ...
              send({
                type: 'tool',
                status: 'call',
                name: core.toolBrief(event, payload.view, 300) || event.data?.name || 'tool',
              ...
              send({
                type: 'tool',
                status: 'result',
                name: core.toolBrief(event, payload.view, 300) || event.data?.name || '',
              });
